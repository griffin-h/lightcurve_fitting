 ...
 * Fitting the Planck function using :func:`scipy.optimize.curve_fit`. This is very fast but may not give reliable uncertainties.
    ...
    * The Stefan-Bolzmann law gives the total bolometric luminosity, ``L_bol`` and ``dL_bol``.
 ...
 * Fitting the Planck function using an MCMC routine.
   ...
   The columns ``temp_mcmc``, ``radius_mcmc``, ``dtemp_mcmc0``, ``dtemp_mcmc1``, ``dradius_mcmc0``, ``dradius_mcmc1`` come from this fit.
   My convention for non-Gaussian uncertainties is that 0 is the lower uncertainty and 1 is the upper uncertainty.

    * The Stefan-Bolzmann law gives the total bolometric luminosity, ``L_bol_mcmc``, ``dL_bol_mcmc0`` and ``dL_bol_mcmc1``.
    * Integrating the Planck function between :math:`U` and :math:`I` band (observed) gives ``L_mcmc``, ``dL_mcmc0``, and ``dL_mcmc1``, the pseudobolometric luminosity.
