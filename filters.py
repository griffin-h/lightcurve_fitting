...
class Filter():
    def __init__(self, names, color='k', offset=0, system=None, fnu=None, filename='', angstrom=False, linecolor=None, textcolor=None):
        if type(names)==list:
            self.name = names[0]
            self.names = names
        else:
    ...
    def read_curve(self, show=False, force=False):
        if (self.trans is None or force) and self.filename:
            ...
            if show:
                plt.figure(1)
                ax1 = plt.gca()
                ax1.plot(self.trans['wl'], self.trans['T'], self.color if self.color != 'w' else 'k', label=self.system + ' ' + self.name)
                ax1.errorbar(wl_eff.value, i, xerr=[[wl_eff.value-wl0], [wl1-wl_eff.value]], marker='o', **self.plotstyle)
                ax1.set_xlabel('Wavelength (nm)')
                ax1.set_ylabel('Transmission')

            dfreq = np.trapz(self.trans['T'].quantity, self.trans['freq'].quantity)
            freq_eff = np.trapz(self.trans['T'].quantity * self.trans['freq'].quantity, self.trans['freq'].quantity) / dfreq
            ...
            if show:
                plt.figure(2)
                ax2 = plt.gca()
                ax2.plot(self.trans['freq'], self.trans['T'], self.color if self.color != 'w' else 'k', label=self.system + ' ' + self.name)
                ax2.errorbar(freq_eff.value, i, xerr=[[freq_eff.value-freq0], [freq1-freq_eff.value]], marker='o', **self.plotstyle)
    ...
    def __repr__(self): return '<filter '+self.name+'>'
...
all_filters = [Filter(['UVW2', 'uvw2', 'W2', '2', 'uw2'], '#FF007F', 8, 'Swift', 7.379e-24, filename='Swift_UVOT.UVW2.dat', angstrom=True),
               ...
               Filter('U', '#3C0072', 4, 'Johnson', 1.79e-23, filename='jnsn-uv-183.asci'),
