...
#def groupLC(self, *groupby):
#    if self.lc is not None:# raise Exception('must run sn.readLC() first')
#      if groupby:
#        self.groupby = groupby
#      groupby = list(set(self.groupby) & set(self.lc.keys()))
#      if 'source' in groupby and self.lc.masked:
#        self.lc['source'][self.lc['source'].mask] = 'other'
#      if groupby:
#        self.lc = self.lc.group_by(groupby)

    def bin(self, delta=0.3, groupby=['filt', 'source']):
        subtabs = []
    ...
    def calcLum(self, nondetSigmas=3):
        self.nondetSigmas = nondetSigmas
        self['lum'], self['dlum'] = mag2flux(self['absmag'], self['dmag'], self.zp() + 90.19, self['nondet'], self.nondetSigmas)
...
#      print self.name, self.peakdate, self.peakcriteria
    ...
    def plot(self, xcol='phase', ycol='absmag', offset_factor=1, color='filt', marker='source', use_lines=False, normalize=False, fillmark=True, **criteria):
        xchoices = ['phase', 'MJD']
        ...
        if np.any(plotthese):
            ...
            return
        groupby = []
        if color in plottable.keys(): groupby.append(color)
        if marker in plottable.keys(): groupby.append(marker)
        if groupby: plottable = plottable.group_by(groupby)
        for g in plottable.groups:
          ...
          elif color in plottable.keys():
            col = colors[g[color][0]]
            mec = col if col not in ['w', '#FFFFFF'] else 'k'
          else:
            col = 'k'
            mec = 'k'
          ...
          if marker=='name':
            ...
            if g[marker][0] not in LC.markers:
              LC.markers[g[marker][0]] = LC.othermarkers.next()
          ...
          if not use_lines:
            ...
            plt.plot(x[~g['nondet']], y[~g['nondet']], color=mec, mfc=mfc, mec=mec, marker=mark, label=label, linestyle=self.sn.linestyle, linewidth=self.sn.linewidth)
            plt.plot(x[g['nondet']], y[g['nondet']], color=mec, mfc=mfc, mec=mec, marker=mark, linestyle='none')
          else:
            plt.plot(x, y, color=mec, mfc=mfc, mec=mec, marker=mark, label=label, linestyle=self.sn.linestyle, linewidth=self.sn.linewidth)

    @classmethod
    def read(self, filepath, format='ascii.fixed_width', fill_values=[('--', '0'), ('', '0')], **kwargs):
        t = super(LC, self).read(filepath, format=format, fill_values=fill_values, **kwargs)
